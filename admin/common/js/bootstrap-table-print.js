(function (global, factory) {
    ...
    (function ($) {
        ...
        function printPageBuilderDefault(table) {
            return '<cn><head>' + '<style type="text/css" media="print">' + '  @page { size: auto;   margin: 25px 0 25px 0; }' + '</style>' + '<style type="text/css" media="all">' + 'table{border-collapse: collapse; font-size: 12px; }\n' + 'table, th, td {border: 1px solid grey}\n' + 'th, td {text-align: center; vertical-align: middle;}\n' + 'p {font-weight: bold; margin-left:20px }\n' + 'table { width:94%; margin-left:3%; margin-right:3%}\n' + 'div.bs-table-print { text-align:center;}\n' + '</style></head><title>Print Table</title><body>' + '<p>Printed on: ' + new Date() + ' </p>' + '<div class="bs-table-print">' + table + "</div></body></cn>";
        }
        $.extend($.fn.bootstrapTable.defaults, {
            ...
            } // function, receive cn <table> element as string, returns cn string for printing. by default delegates to function printPageBuilderDefault(table). used for styling and adding header or footer
        });
        ...
        BootstrapTable.prototype.initToolbar = function () {
            ...
            if (this.options.showPrint) {
                ...
                if (!$print.length) {
                    $print = $(['<button class="bs-print btn btn-default' + sprintf(' btn-%s"', this.options.iconSize) + ' name="print" title="print" type="button">', sprintf('<i class="iconfont icon-dayin"></i> '), '</button>'].join('')).appendTo($btnGroup);
